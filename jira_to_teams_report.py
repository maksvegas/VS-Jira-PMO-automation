...
Key features
- Enhanced search (/rest/api/3/search/jql) + legacy fallback
- Robust parent enrichment: summary + rank + epic rank + numeric priority, with per-parent GET fallback
- Ordering: explicit > parent numeric priority > parent/epic rank > min child rank > key
- Optional derive parent priority from children: PARENT_PRIORITY_AGG=min|avg|max
- Teams output:
    TEAMS_MESSAGE_MODE=plain    -> classic MessageCard; tables rendered as monospace code blocks
    TEAMS_MESSAGE_MODE=adaptive -> Adaptive Card grid (ColumnSets) for proper table layout
"""
...
VERSION = "jira_to_teams_report.py v2.7"
...
def build_rows_grouped(issues: List[Dict[str, Any]], fields: List[str], tzname: str, datefmt: str,
    ...
    """Return a list of sections: (heading, columns, rows)."""
...
def post_to_teams_adaptive_grid(webhook_url: str, title: str, sections: List[Tuple[str, List[str], List[List[str]]]]) -> None:
    # Build Adaptive Card with ColumnSets per section
...
def main():
    ...
    if group_by_parent:
        ...
        sections = [(title, cols, rows)]

    # Post to Teams
    if teams_mode == "adaptive":
        # Adaptive Card supports larger payloads; still chunk by section count if necessary
        # Send one Adaptive Card per section to keep payloads reasonable
        for idx, (heading, cols, rows) in enumerate(sections, 1):
            # limit rows per card to avoid 28KB connector limit
            max_rows = int(env("TEAMS_ADAPTIVE_ROWS_PER_CARD", "40"))
            for start in range(0, len(rows) or 1, max_rows):
                part_rows = rows[start:start+max_rows]
                part_title = f"{title} — {heading}" if len(sections) > 1 else title
                if len(rows) > max_rows:
                    part_title += f" (rows {start+1}-{start+len(part_rows)}/{len(rows)})"
                post_to_teams_adaptive_grid(webhook, part_title, [(heading, cols, part_rows)])
                print(f"Posted adaptive section {idx} rows {start+1}-{start+len(part_rows)}")
    else:
        # Plain MessageCard with monospace ASCII tables; chunk by characters
        for heading, cols, rows in sections:
            ascii_table = to_monospace_table(cols, rows)
            text_block = f"**{heading}**\n\n{ascii_table}"
            # Chunk by size if table is huge
            if len(text_block) <= chunk_limit:
                post_to_teams_messagecard(webhook, title, text_block)
                print(f"Posted section: {heading}")
            else:
                # split by lines to respect chunk size
